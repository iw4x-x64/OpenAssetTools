    ...
    public:
        ...
        void PerformStep(ZoneLoader& zoneLoader, ILoadingStream& stream) override
        {
            const auto inputStream = ZoneInputStream::Create(
                m_pointer_bit_count, m_block_bit_count, m_offset_bit_count, zoneLoader.m_blocks, m_insert_block, stream, m_memory, std::move(m_progress_callback));
...
namespace step
{
                                                            ...
                                                            std::optional<std::unique_ptr<ProgressCallback>> progressCallback)
    {
        return std::make_unique<StepLoadZoneContent>(
            std::move(entryPointFactory), pointerBitCount, blockBitCount, offsetBitCount, insertBlock, memory, std::move(progressCallback));
    }
} // namespace step
