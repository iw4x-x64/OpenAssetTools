...
#include "Game/IW4MS/AssetWriterIW4MS.h"
...
static_assert(sizeof(void*) == 8u, "IW4MS zones can only be written by a 64 bit build");
...
void ContentWriter::WriteScriptStringList(const bool atStreamStart)
{
    ...
    if (varScriptStringList->strings != nullptr)
    {
        ...
        static_assert(offsetof(ScriptStringList, strings) == 8u);
        m_stream->MarkFollowing(varScriptStringListWritten.AtOffset(8));
    }
}

void ContentWriter::WriteXAsset(const bool atStreamStart)
{
    static_assert(offsetof(XAsset, header.data) == 8u);

#define WRITE_ASSET(type_index, typeName, headerEntry)                                                                                                         \
    case type_index:                                                                                                                                           \
    {                                                                                                                                                          \
        Writer_##typeName writer(varXAsset->header.headerEntry, m_zone, *m_stream);                                                                            \
        writer.Write(varXAsset->header.headerEntry, varXAssetWritten.AtOffset(8));                                                                             \
        break;                                                                                                                                                 \
    }
...
void ContentWriter::WriteXAssetArray(const bool atStreamStart, const size_t count)
{
    ...
    static_assert(sizeof(XAsset) == 16u);
...
void ContentWriter::WriteContent(ZoneOutputStream& stream)
{
    ...
    if (varXAssetList->assets != nullptr)
    {
        ...
        m_stream->MarkFollowing(varXAssetListWritten.AtOffset(24));
    }
