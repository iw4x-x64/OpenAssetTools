#include "MenuListDumperIW4MS.h"

#include "Game/IW4MS/Menu/MenuDumperIW4MS.h"
#include "Menu/AbstractMenuWriter.h"
#include "MenuWriterIW4MS.h"
...
namespace
{
    ...
    void DumpFunctions(menu::IWriterIW4MS& menuDumper, const MenuList* menuList)
    {
    ...
    void DumpMenus(menu::IWriterIW4MS& menuDumper, menu::MenuDumpingZoneState* zoneState, const MenuList* menuList)
    {
...
namespace menu
{
    void CreateDumpingStateForMenuListIW4MS(MenuDumpingZoneState* zoneState, const MenuList* menuList)
    {
    ...
    void MenuListDumperIW4MS::DumpAsset(AssetDumpingContext& context, const XAssetInfo<AssetMenuList::Type>& asset)
    {
        ...
        auto menuWriter = CreateMenuWriterIW4MS(*assetFile);
    ...
    void MenuListDumperIW4MS::Dump(AssetDumpingContext& context)
    {
        auto* zoneState = context.GetZoneAssetDumperState<MenuDumpingZoneState>();

        auto menuListAssets = context.m_zone.m_pools.PoolAssets<AssetMenuList>();
        for (const auto* asset : menuListAssets)
            CreateDumpingStateForMenuListIW4MS(zoneState, asset->Asset());
