...
namespace IW4MS
{
    ...
    public:
        ...
        // Serialized pointers are 64 bit, but the block index does not sit at the top of them.
        // DB_ConvertOffsetToPointer at 0x14012DDC0 resolves a pointer as
        //     blockBase[(v - 1) >> 28] + ((v - 1) & 0xFFFFFFF)
        // so the offset occupies 28 bits, the block index the four bits directly above it, and the
        // entire upper half of the pointer is unused.
        static constexpr unsigned POINTER_BIT_COUNT = 64u;
        static constexpr unsigned OFFSET_BLOCK_BIT_COUNT = 4u;
        static constexpr unsigned OFFSET_BIT_COUNT = 28u;
