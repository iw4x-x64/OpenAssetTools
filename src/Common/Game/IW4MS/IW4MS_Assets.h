...
#ifndef __IW4MS_ASSETS_H
#define __IW4MS_ASSETS_H
...
namespace IW4MS
{
    ...
    typedef tdef_align64(16) char raw_byte16;
    typedef tdef_align64(16) float raw_float16;
    typedef unsigned int raw_uint;
    typedef tdef_align64(128) unsigned int raw_uint128;
    ...
    typedef tdef_align64(4) unsigned short UShortVec[3];
    ...
    // IW4 x86 forces this struct to 4-byte alignment. Under the
    // Microsoft x64 ABI the embedded XAnimDynamicFrames union holds
    // pointers, so the natural alignment is 8 and the forced value
    ...
    // Same as XAnimPartTransFrames: the XQuat2* member makes this
    // 8-aligned on x64, so the x86 forced 4-byte alignment is dropped.
    ...
    typedef tdef_align64(16) XSurfaceTri XSurfaceTri16;
    ...
    // The other structure that genuinely differs from IW4 x86, so again the evidence is written out.
    // The x86 model is:
    //
    //     struct water_t {
    //         WaterWritable writable;   // { float floatTime; }
    //         complex_s* H0;            // { float real; float imag; }, so 8 bytes per element
    //         float* wTerm;
    //         int M, N;
    //         float Lx, Lz, gravity, windvel, winddir[2], amplitude, codeConstant[4];
    //         GfxImage* image;
    //     };
    //
    // Compiled for x64 that is 88 bytes with H0 at +8, wTerm at +16, M at +24 and image at +80.
    // Load_water_t at 0x140123EA0 disagrees on every one of those:
    //
    //     140123ea0  sub     rsp, 28h
    //     140123ea4  mov     rdx, cs:qword_140DFEB28
    //     140123eab  mov     r8d, 60h                 ; (1) body is 0x60 = 96 bytes, not 88
    //     140123eb1  call    Load_Stream
    //     140123eb6  mov     rax, cs:qword_140DFEB28
    //     140123ebd  cmp     qword ptr [rax+8], 0     ; first array
    //     140123ec2  jz      short loc_140123F05
    //     140123ec4  mov     ecx, 3
    //     140123ec9  call    DB_AllocStreamPos        ; align 4
    //     140123ece  mov     rdx, rax
    //     140123ed1  mov     cl, 1
    //     140123ed3  mov     rax, cs:qword_140DFEB28
    //     140123eda  mov     [rax+8], rdx
    //     140123ede  mov     rax, cs:qword_140DFEB28
    //     140123ee5  mov     cs:qword_140DFD688, rdx
    //     140123eec  mov     r8d, [rax+24h]           ; (2) N at +36
    //     140123ef0  imul    r8d, [rax+20h]           ;     times M at +32
    //     140123ef5  shl     r8d, 2                   ;     times 4: M * N floats
    //     140123ef9  call    Load_Stream
    //     ...      the identical sequence repeats for [rax+10h] and [rax+18h]  ; (3) THREE arrays
    //     140123f95  add     rax, 58h                 ; (4) image at +88
    //     140123f9b  mov     cs:qword_140DFE930, rax
    //     140123fa6  jmp     Load_GfxImagePtr
    //
    //  (1) the body is 96 bytes.
    //  (2) the counts come from +32 and +36, so M and N are there, not at +24 and +28.
    //  (3) there are three array pointers, at +8, +16 and +24. +24 receives an allocated pointer,
    //      so it cannot be `int M` as the x86 layout would have it.
    //  (4) the image is at +88.
    //
    // Which of the three arrays is which follows from arithmetic rather than from the loader. All
    // three are read as `M * N * 4`, so none of them can be x86's H0: that is an interleaved
    // complex_s array and would be `M * N * 8`. But two of them together are exactly `M * N * 8`.
    // So this build stores H0's real and imaginary parts as two planar float arrays instead of one
    // interleaved array, and the third array is wTerm, which matches x86 in both size and relative
    // position. Total stream consumed is `M * N * 12` either way.
    //
    // Which planar half is the real part is not established, so those two stay numbered.
    //
    struct water_t
    {
    ...
    struct gcc_align64(8) GfxDrawSurfFields
    {
    ...
    union GfxDrawSurf
    {
        gcc_align64(8) GfxDrawSurfFields fields;
        gcc_align64(8) uint64_t packed;
    };
    ...
    struct type_align64(4) GfxImageLoadDef
    {
        char levelCount;
        char pad[3];
        int flags;
    ...
    // This structure differs from IW4 x86. The x86 model is:
    //
    //     struct MSSSpeakerLevels { int speaker; int numLevels; float levels[2]; };
    //
    // which is also 16 bytes, so the size alone proves nothing. What settles it is the loader,
    // Load_MSSChannelMap at 0x14011C2D0, which walks a 32 byte element as two 16 byte entries:
    //
    //     14011c320  mov     r8d, 10h                 ; 16 byte entry
    //     14011c326  mov     cs:qword_140DFE458, rbx
    //     14011c32d  mov     rdx, rbx
    //     14011c330  xor     ecx, ecx
    //     14011c332  call    Load_Stream
    //     14011c337  mov     rax, cs:qword_140DFE458
    //     14011c33e  cmp     qword ptr [rax+8], 0     ; (1) +8 is tested as a QWORD
    //     14011c343  jz      short loc_14011C37A
    //     14011c345  mov     ecx, 3
    //     14011c34a  call    DB_AllocStreamPos        ; align 4
    //     14011c34f  mov     rdx, rax
    //     14011c352  mov     cl, 1
    //     14011c354  mov     rax, cs:qword_140DFE458
    //     14011c35b  mov     [rax+8], rdx             ; (2) +8 RECEIVES the allocated pointer
    //     14011c35f  mov     rax, cs:qword_140DFE458
    //     14011c366  mov     cs:qword_140DFD688, rdx
    //     14011c36d  movzx   r8d, byte ptr [rax]      ; (3) count is a BYTE load at +0
    //     14011c371  shl     r8d, 3                   ;     times 8
    //     14011c375  call    Load_Stream              ;     read count * 8 bytes
    //     14011c37a  add     rbx, 10h                 ; next entry, stride 16
    //     14011c37e  sub     rdi, 1
    //     14011c382  jnz     short loc_14011C320      ; exactly two entries
    //
    // Three separate facts fall out of that, and each one on its own rules out the x86 layout:
    //
    //  (1)+(2) +8 holds a pointer. The loader tests it as a qword and then writes an allocation
    //          result into it. It cannot be `float levels[2]`, which would be overwritten.
    //  (3)     the count is a byte at +0, not an int at +4. A compiler does not narrow an int32
    //          load to `movzx ...byte` when the value feeds a size computation, because that would
    //          be wrong for counts above 255. The byte load means the field is byte sized.
    //
    // So the levels are behind a pointer here and inline in x86, and the count moved to +0.
    //
    // What the seven bytes after the count are is genuinely unknown. Observing them zero does not
    // settle it: an `int speaker` of 2 followed by an `int numLevels` of 0 also reads as a count
    // byte followed by seven zeroes. They are zero in all 20 entries present in retail mp_rust,
    // which is every entry in that zone, alongside a set level pointer and a count drawn from
    // {2, 4, 6, 12}. Those four values are what a 2x2 grid of source channels against output
    // speakers produces for mono and stereo sources feeding stereo and 5.1 outputs: 1x2, 1x6, 2x2
    // and 2x6, which is at least consistent with the count being a level count.
    //
    // Note that Load_SpeakerMap and Load_MSSChannelMap have no callers outside the loader chain,
    // so nothing ever reads these fields back. The reference implementation reached the same shape
    // independently and also left them unnamed.
    //
    // The level entries themselves did decode, from the data rather
    // than the code. Dumping them out of retail mp_rust gives, for a
    // count of 4:
    //
    //     00 00 00 00 | 00 00 80 3f      (0, 0) = 1.0
    //     01 00 00 00 | 00 00 00 00      (1, 0) = 0.0
    //     00 01 00 00 | 00 00 00 00      (0, 1) = 0.0
    //     01 01 00 00 | 00 00 80 3f      (1, 1) = 1.0
    //
    // which is an identity stereo mix matrix. The count of 6 is one
    // source channel against speakers 0 to 5 at 0.5, 0.5, 0, 0, 0, 0,
    // and the count of 12 is two source channels against those same six
    // speakers. So an entry is a source channel index, a speaker index,
    // two unused bytes and a float gain, and levelCount is the number
    // of such triples. vec2_t is used only because the element is 8
    // bytes.
    struct MSSSpeakerLevel
    {
    ...
    struct MSSSpeakerLevels
    {
        ...
        unsigned char unknown_1[7];
        MSSSpeakerLevel* levels;
    };

    // 32 bytes, walked by Load_MSSChannelMap as exactly two MSSSpeakerLevels entries. The index is
    // the speaker configuration: 0 is two speakers, 1 is six.
    ...
    struct SpeakerMap
    {
        bool isDefault;
        const char* name;
        // Load_SpeakerMap at 0x14011C200 reads an 80 byte body and takes the name pointer at +8. It
        // also reads 64 bytes at +16 which it walks as two MSSChannelMap elements of 32 bytes each.
        // The IW4 model nests an MSSChannelMap holding six speakers here, which would make the body
        // 416 bytes and over-read every alias by 336.
        //
        // The index is the source channel count minus one: 0 is mono, 1 is stereo.
    ...
    struct DynEntityClient
    {
        ...
        // Retail allocates 16 bytes per entry, not the 12 the fields above account for. In
        // Load_clipMap_t at 0x1401217D0 both dynEntClientList arrays are sized with "shl r8d, 4"
        // against dynEntCount, where the pose list beside them uses "shl 5" for 32 and the coll list
        // "lea r8d, [r8+r8*4]; shl r8d, 2" for 20. Both of which match, but only this one is short.
        //
        // I'm not sure what the 4 bytes hold, but the array is in the runtime
        // block, which is memset rather than read, so nothing in the fastfile describes it and no
        // loader touches it, so only the size has any effect, hence a trailing run rather than an
        // invented field somewhere in the middle.
        char unknown_12[4];
    };
    ...
    typedef tdef_align64(128) cbrush_t cbrush_array_t;
    typedef tdef_align64(128) Bounds BoundsArray;
    ...
    struct clipMap_t
    {
        ...
        // Trailing runtime state the loader never reads. Sized so the struct reaches the 512 bytes
        // that Load_clipMap_t reads at 0x1401217D0, and that DB_GetXAssetTypeSize reports for types
        // 14 and 15. The x86 model uses 48 here and very field before this point keeps the same
        // offsets, so the tail is the only difference.
        char padding[108];
    };
    ...
    struct FxGlassPieceDynamics
    {
        ...
        // Retail allocates 48 bytes per entry, not the 36 the fields
        // above account for: sub_140116C60 sizes the array as "48 *
        // pieceLimit". The inherited IW4 struct is 12 bytes short. Like
        // above it's not clear what those 12 bytes hold, and nothing in
        // the fastfile describes it and no loader touches it.
        char unknown_36[12];
    };
    ...
    struct type_align64(4) GfxBrushModel
    {
    ...
    typedef tdef_align64(128) GfxCellTree GfxCellTree128;
    typedef tdef_align64(4) GfxSceneDynModel GfxSceneDynModel4;
